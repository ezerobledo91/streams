...
async function loadManifest(source, force = false) {
  ...
  const payload = await fetchJson(source.manifestUrl, { timeoutMs: 10000 });
...
async function fetchTmdbCatalogItems(category, query, page, limit, genreId = "") {
  ...
  if (normalizedGenre && /^\d+$/.test(normalizedGenre)) {
    ...
    return dedupeItems(
      merged.map((entry) => buildTmdbItem(entry, tmdbType)),
      limit
    );
  }

  if (query) {
    const payload = await fetchTmdb(`/search/${tmdbType}`, {
      query,
      language,
      page: String(page),
      include_adult: "false"
    ...
    const results = Array.isArray(payload?.results) ? payload.results : [];
    return dedupeItems(
      results.slice(0, Math.max(limit * 2, 120)).map((entry) => buildTmdbItem(entry, tmdbType)),
      limit
    );
  }
...
async function fetchSourceCatalogItems(source, category, query, page, perSourceLimit) {
  try {
    const manifest = await loadManifest(source);
    const catalogs = pickCatalogs(manifest, category, query);
    ...
    for (const catalog of catalogs) {
      ...
      for (const meta of metas) {
        items.push(normalizeCatalogItem(meta, category, source, catalogId));
      }
    }
