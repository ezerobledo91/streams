const { safeText, compactText, nowMs, buildStreamSourceKey } = require("../utils");
...
const { pickTorrentFile } = require("./files");
...
async function createPlaybackSession({
  ...
  torrent.on("ready", () => {
    try {
      ...
      if (shouldUseHlsForFileName(file?.name, PLAYBACK_HLS_ENABLED, PLAYBACK_HLS_FORCE, BROWSER_NATIVE_VIDEO_EXTENSIONS)) {
