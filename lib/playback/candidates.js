...
function extractCandidateLanguageHints(stream) {
  ...
  // Detectar cirílico (ruso/ucraniano) por caracteres en el texto
  if (/[\u0400-\u04FF]{3,}/.test(text) && !hints.has("ru")) {
    hints.add("ru");
  }
...
function isLikelyUnavailableStream(stream) {
  ...
  return (
    ...
    url.includes("public-rate-limit") ||
    text.includes("[⛔")
  );
}
...
function calculateScore(stream, providerId, audioOptions = {}) {
  ...
  let allowedLanguage = hasPreferredLanguageHint(languageHints, allowedLanguageSet);
  let languageRank = resolveLanguageRank(languageHints, audioPriority);
  if (!languageHints.size) {
...
function rankCandidateForAuto(candidate) {
  ...
  const videoSizeBytes = Number(candidate?.videoSizeBytes || 0);
  const sizeGb = videoSizeBytes / (1024 * 1024 * 1024);
  const seeders = Number(candidate?.seeders || 0);
  rank -= reliabilityPenalty * 1.5;

  if (sizeGb > 0 && sizeGb < 1.5) rank += 30;
  else if (sizeGb < 3) rank += 15;
  rank -= sizeGb * 8;
  if (sizeGb > 8) rank -= 40;
  if (sizeGb > 14) rank -= 60;
  ...
  if (seeders >= 50) rank += 25;
  else if (seeders >= 20) rank += 15;
  else if (seeders >= 10) rank += 8;

  if (candidate?.resolution >= 2160) rank += 3;
  else if (candidate?.resolution >= 1080) rank += 5;
  else if (candidate?.resolution >= 720) rank += 7;
  else rank += 4;

  if (candidate?.likelyIncompatible) rank -= 8;
  if (videoSizeBytes > 20 * 1024 * 1024 * 1024) rank -= 6;
  if (seeders <= 0 && (candidate?.peers || 0) <= 0) rank -= 6;
...
function buildCandidatesFromResults(results, audioOptions = {}) {
  ...
  console.log(`[CANDIDATES-DEBUG] total=${out.length} allowedLang=${preferredOnly.length} filteredOut=${out.length - preferredOnly.length}`);
  if (preferredOnly.length) {
    const deduped = dedupeCandidates(preferredOnly);
    console.log(`[CANDIDATES-DEBUG] afterDedup=${deduped.length} (usando solo allowedLanguage)`);
    return deduped;
  }
  const deduped = dedupeCandidates(out);
  console.log(`[CANDIDATES-DEBUG] afterDedup=${deduped.length} (usando todos, ninguno tenia allowedLanguage)`);
  return deduped;
}
