...
const CIRCUIT_EXEMPT_PROVIDERS = new Set(["m3u-vod"]);

function isCircuitBreakerExempt(providerId) {
  const id = normalizeProviderId(providerId);
  return CIRCUIT_EXEMPT_PROVIDERS.has(id);
}
...
function isProviderCircuitOpen(providerId) {
  if (isCircuitBreakerExempt(providerId)) return false;
...
function buildProviderReliabilityPenalty(providerId) {
  ...
  const circuitExempt = isCircuitBreakerExempt(providerId);
  ...
  const breakerOpen = !circuitExempt && Number(stats.breakerUntil || 0) > now;
  if (breakerOpen) {
    penalty += 80;
  }
...
function registerPlaybackOutcome(providerId, sourceKey, success, reason = "") {
  ...
  const circuitExempt = isCircuitBreakerExempt(normalizedPid);
  const now = nowMs();
  ...
  if (success) {
    ...
    if (!circuitExempt && providerStats.consecutiveFailures >= RELIABILITY_CIRCUIT_THRESHOLD) {
  ...
  if (circuitExempt && providerStats.breakerUntil) {
    providerStats.breakerUntil = 0;
  }
