...
// Detecta si una URL de IPTV Xtream es live, película o serie
function detectIptvContentType(url) {
  const raw = String(url || "");
  if (/\/movie\//i.test(raw)) return "movie";
...
function normalizeLiveCategoryId(rawValue, fileBaseName = "") {
  ...
  if (normalized.includes("novela") || normalized.includes("telenovela")) return "novelas";
  ...
  if (normalized.includes("evento") || normalized.includes("ppv")) return "eventos";
  if (normalized.includes("documental")) return "documentales";
  if (normalized.includes("relig")) return "religion";
  // Países/regiones comunes
  if (normalized.includes("argentina")) return "argentina";
  if (normalized.includes("mexico") || normalized.includes("mexicana") || normalized.includes("mexicano")) return "mexico";
  if (normalized.includes("colombia") || normalized.includes("colombiana")) return "colombia";
  if (normalized.includes("chile") || normalized.includes("chilena")) return "chile";
  if (normalized.includes("peru") || normalized.includes("peruana")) return "peru";
  if (normalized.includes("venezuela")) return "venezuela";
  if (normalized.includes("ecuador")) return "ecuador";
  if (normalized.includes("usa") || normalized.includes("estados unidos")) return "usa";
...
function parseM3uExtinf(line) {
  ...
  const commaIndex = findExtinfTitleSeparator(body);
...
function findExtinfTitleSeparator(body) {
...
function isLikelyBrowserPlayableHttpUrl(value) {
  const url = safeText(value).toLowerCase();
  if (!/^https?:\/\//i.test(url)) return false;
  if (/\.m3u8(?:$|\?)/i.test(url)) return true;
  if (/\.(mp4|m4v|webm|mpd)(?:$|\?)/i.test(url)) return true;
  if (/\/(?:playlist|master|manifest)(?:$|\?)/i.test(url)) return true;
  return false;
}
...
function normalizeLiveChannel(raw, fileBaseName, sourceFileName = `${fileBaseName}.m3u`) {
  ...
  const contentType = detectIptvContentType(streamUrl);
  ...
  const channel = {
    ...
    streamUrl,
    webPlayable: isLikelyBrowserPlayableHttpUrl(streamUrl),
    ...
    httpReferrer: safeText(raw.httpReferrer) || null,
    httpUserAgent: safeText(raw.httpUserAgent) || null,
    ...
    const channel = normalizeLiveChannel(
      {
        ...
        tvgId: attrs["tvg-id"],
        httpReferrer: attrs["http-referrer"] || attrs["referer"] || attrs["referrer"],
        httpUserAgent: attrs["http-user-agent"] || attrs["user-agent"],
        logo: attrs["tvg-logo"],
