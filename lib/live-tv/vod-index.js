...
function tokenizeNormalizedText(value) {
  ...
  const rawTokens = normalizeLooseText(value)
    .split(" ")
  ...
  for (const token of rawTokens) {
    ...
    if (acronymBuffer.length >= 2) {
      mergedTokens.push(acronymBuffer);
    }
    acronymBuffer = "";
...
function titleSimilarityRatio(candidateTitle, queryTitle) {
  const queryTokens = tokenizeNormalizedText(queryTitle);
  if (!queryTokens.length) {
    return normalizeLooseText(candidateTitle) === normalizeLooseText(queryTitle) ? 1 : 0;
  }
  const candidateTokenSet = new Set(tokenizeNormalizedText(candidateTitle));
  if (!candidateTokenSet.size) return 0;
  let matches = 0;
...
function searchVodIndex(title, { season = "", episode = "" } = {}) {
  ...
  if (hasEpisodeTarget) {
    ...
    // Si no hay match exacto por titulo normalizado, mantener modo estricto por episodio.
    // Evita recomendar episodios incorrectos o peliculas con titulos cortos (ej: "The").
    if (!candidates.length) {
      candidates = vodState.entries.filter((entry) => {
        if (entry.season !== seasonNum || entry.episode !== episodeNum) return false;
        if (entry.normalizedTitle === normalizedTitle) return true;
        const compactEntry = compactNormalizedText(entry.normalizedTitle);
        const compactQuery = compactNormalizedText(normalizedTitle);
        if (
          compactEntry.length >= 5 &&
          compactQuery.length >= 5 &&
          (compactEntry.includes(compactQuery) || compactQuery.includes(compactEntry))
        ) {
          return true;
        }
        if (
        ...
        return titleSimilarityRatio(entry.normalizedTitle, normalizedTitle) >= 0.5;
      });
    }

    // Para series con episodio objetivo no hacemos fallback amplio por titulo:
    // es preferible devolver vacio antes que reproducir episodio incorrecto.
