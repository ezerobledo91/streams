...
function buildPreflightCacheKey({ type, itemId, season, episode, quality, maxCandidates }) {
  ...
  return `${normalizedType}|${normalizedItem}|${normalizedSeason}|${normalizedEpisode}|${normalizedQuality}|${Number(maxCandidates || 0)}`;
}
...
function registerPlaybackRoutes(app) {
  ...
  app.post("/api/playback/preflight", async (req, res) => {
    ...
    const candidates = buildCandidatesFromResults(aggregated.results);
  ...
  app.post("/api/playback/auto", async (req, res) => {
    ...
    const preferredSourceKey =
      ...
      "";
    const waitReadyMs = clamp(Number.parseInt(req.body?.waitReadyMs || req.query.waitReadyMs, 10) || 30000, 8000, 120000);
    const probeTimeoutMs = clamp(Number.parseInt(req.body?.probeTimeoutMs || req.query.probeTimeoutMs, 10) || 6000, 2000, 15000);
    const maxCandidates = clamp(Number.parseInt(req.body?.maxCandidates || req.query.maxCandidates, 10) || 24, 4, 80);
    const validationBudgetMs = clamp(
      Number.parseInt(req.body?.validationBudgetMs || req.query.validationBudgetMs, 10) || 22000,
      5000,
      60000
    );
    ...
    const orderedWithPreferred = preferredSourceKey
      ...
      : ordered;
    const directCandidates = orderedWithPreferred.filter((candidate) => Boolean(candidate.directUrl));
    const fallbackCandidates = orderedWithPreferred.filter((candidate) => Boolean(candidate.magnet));
    const attempts = [];
    const deadlineAt = Date.now() + validationBudgetMs;
    const qualityOrder = ["4k", "1080p", "720p", "sd"];
    const pickAvailableQualities = (entriesByQuality) => qualityOrder.filter((item) => entriesByQuality.has(item));
    const estimateAutoEntryRank = (entry) => {
      const candidate = entry?.candidate || {};
      const sizeGb = Number(candidate.videoSizeBytes || 0) / (1024 * 1024 * 1024);
      const qualityBonus =
        entry?.bucket === "4k" ? 24 : entry?.bucket === "1080p" ? 16 : entry?.bucket === "720p" ? 10 : 4;
    ...
    const pickAutoEntry = (entriesByQuality) => {
      let bestEntry = null;
      let bestRank = Number.NEGATIVE_INFINITY;
    ...
    const pickSelectedEntry = (entriesByQuality, availableQualities) => {
      if (normalizedQuality !== "auto") {
        return entriesByQuality.get(normalizedQuality) || null;
      }
      return entriesByQuality.get(availableQualities[0]) || null;
    ...
    const validatedDirectByQuality = new Map();
    const maxDirectProbeCandidates = normalizedQuality === "auto" ? 6 : 10;
    let directProbesTried = 0;
    for (const candidate of directCandidates) {
      if (Date.now() >= deadlineAt) break;
      if (directProbesTried >= maxDirectProbeCandidates) break;
      const qualityBucket = qualityBucketFromResolution(candidate.resolution);
      if (validatedDirectByQuality.has(qualityBucket)) continue;
      directProbesTried += 1;
      ...
      validatedDirectByQuality.set(qualityBucket, {
      ...
      if (validatedDirectByQuality.size >= 4) break;
    }

    if (validatedDirectByQuality.size > 0) {
      const availableQualities = pickAvailableQualities(validatedDirectByQuality);
      if (normalizedQuality !== "auto" && !validatedDirectByQuality.has(normalizedQuality)) {
      ...
      const selectedEntry = pickSelectedEntry(validatedDirectByQuality, availableQualities);
      if (!selectedEntry) {
    ...
    const validatedFallbackByQuality = new Map();
    const validatedFallbackNativeByQuality = new Map();
    const attemptedFallbackSources = new Set();
    const initialFallbackAttempts = normalizedQuality === "auto" ? 3 : 6;
    const initialFallbackWaitMs =
      normalizedQuality === "auto" ? clamp(Math.min(waitReadyMs, 12000), 6000, waitReadyMs) : waitReadyMs;
    const rescueFallbackWaitMs =
      normalizedQuality === "auto" ? clamp(Math.min(waitReadyMs, 22000), 8000, waitReadyMs) : waitReadyMs;

    const upsertValidatedFallbackEntry = (entry, qualityBucket) => {
      const current = validatedFallbackByQuality.get(qualityBucket);
      let selected = current;
      if (!current) {
        selected = entry;
      } else if (current.streamKind !== entry.streamKind) {
      ...
      if (selected === entry) {
        if (current?.sessionId && current.sessionId !== entry.sessionId) {
          destroySession(current.sessionId, "autoplay-replaced-quality");
        }
        validatedFallbackByQuality.set(qualityBucket, entry);
        if (entry.streamKind === "direct") {
          validatedFallbackNativeByQuality.set(qualityBucket, entry);
        } else {
          validatedFallbackNativeByQuality.delete(qualityBucket);
        }
      } else if (entry.sessionId) {
        destroySession(entry.sessionId, "autoplay-lower-score-quality");
      }
    };

    const tryValidateFallbackCandidate = async (candidate, waitMsForCandidate) => {
      if (
        normalizedQuality === "auto" &&
        !candidate?.webFriendly &&
        Number(candidate?.seeders || 0) <= 0 &&
      ...
      const qualityBucket = qualityBucketFromResolution(candidate.resolution);
      const existing = validatedFallbackByQuality.get(qualityBucket);
      if (existing?.streamKind === "direct") {
        return false;
      }
      if (existing && !candidate.webFriendly) {
        return false;
      }
      ...
      try {
        ...
        const finalSession = await waitForSessionStatusChange(createdSession.id, waitMsForCandidate);
        if (!finalSession) {
          throw new Error("Sesion no encontrada tras crear playback.");
        }
        if (finalSession.status !== "ready") {
          throw new Error(finalSession.error || "Sesion no quedo lista en timeout.");
        }
        ...
        upsertValidatedFallbackEntry(entry, qualityBucket);
        attempts.push({
          mode: "torrent",
          providerId: candidate.providerId,
          displayName: compactText(candidate.displayName, 120),
    ...
    let fallbackCandidatesProcessed = 0;
    for (const candidate of fallbackCandidatesForValidation) {
      if (Date.now() >= deadlineAt) break;
      if (fallbackCandidatesProcessed >= initialFallbackAttempts) break;
      fallbackCandidatesProcessed += 1;
      const attemptKey = buildCandidateSourceKey(candidate) || `${candidate.providerId}:${candidate.displayName}:${candidate.resolution}`;
      if (attemptedFallbackSources.has(attemptKey)) {
        continue;
      }
      attemptedFallbackSources.add(attemptKey);
      await tryValidateFallbackCandidate(candidate, initialFallbackWaitMs);

      if (validatedFallbackNativeByQuality.size >= 4) break;
      if (validatedFallbackByQuality.size >= 4 && validatedFallbackNativeByQuality.size > 0) break;
    }

    if (!validatedFallbackByQuality.size && normalizedQuality === "auto") {
      for (const candidate of fallbackCandidatesForValidation) {
        if (Date.now() >= deadlineAt + rescueFallbackWaitMs) break;
        const attemptKey = buildCandidateSourceKey(candidate) || `${candidate.providerId}:${candidate.displayName}:${candidate.resolution}`;
        if (attemptedFallbackSources.has(attemptKey)) {
          continue;
        }
        attemptedFallbackSources.add(attemptKey);
        await tryValidateFallbackCandidate(candidate, rescueFallbackWaitMs);

        if (validatedFallbackByQuality.size > 0) {
          break;
        }
      }
    }

    const availableQualities = pickAvailableQualities(validatedFallbackByQuality);
    ...
    if (normalizedQuality !== "auto" && !validatedFallbackByQuality.has(normalizedQuality)) {
      for (const entry of validatedFallbackByQuality.values()) {
        if (entry?.sessionId) {
          destroySession(entry.sessionId, "autoplay-quality-not-available");
        }
      }
      return res.status(409).json({
        error: "La calidad solicitada no pudo validarse.",
    ...
    const selectedEntry =
      normalizedQuality === "auto"
        ? pickAutoEntry(validatedFallbackNativeByQuality) || pickAutoEntry(validatedFallbackByQuality)
        : pickSelectedEntry(validatedFallbackByQuality, availableQualities);
    if (!selectedEntry) {
      return res.status(502).json({
        error: "No se pudo determinar stream final validado.",
        availableQualities
    ...
    for (const entry of validatedFallbackByQuality.values()) {
      ...
      destroySession(entry.sessionId, "autoplay-unused-quality");
    }
