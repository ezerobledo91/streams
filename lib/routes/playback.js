...
const MAX_AUTO_ALTERNATIVES = 4;
const MAX_AUTO_VALIDATED = 4; // cuántos candidatos validar en modo auto antes de parar
...
function buildAutoAlternatives(entries, preferredEntry, limit = MAX_AUTO_ALTERNATIVES) {
  ...
  const scored = entries
    .map((entry) => ({
      ...
      rank: rankCandidateForAuto(entry?.candidate)
    }))
...
function registerPlaybackRoutes(app) {
  ...
  app.post("/api/playback/auto", async (req, res) => {
    ...
    const attemptedSources = new Set();
    const maxAttempts = normalizedQuality === "auto" ? MAX_AUTO_VALIDATED + 2 : 8;
    let totalAttempts = 0;
    let autoValidatedCount = 0;
    let directProbes = 0;
    let torrentAttempts = 0;
    const maxDirectProbes = normalizedQuality === "auto" ? 5 : 6;
    const maxTorrentAttempts = normalizedQuality === "auto" ? 5 : 5;
    ...
    for (const candidate of orderedWithPreferred) {
      ...
      // Si ya tenemos un directo validado para este bucket Y ya tenemos suficientes alternativas, skip
      if (validatedNativeByQuality.has(bucket) && autoValidatedCount >= MAX_AUTO_VALIDATED) continue;
      ...
      if (candidate.directUrl) {
        ...
        if (probe.ok) {
          ...
          // En auto, seguir validando hasta tener suficientes alternativas
