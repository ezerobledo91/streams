...
function cleanDisplayName(raw) {
  ...
  // Remover emojis y caracteres unicode decorativos
  text = text.replace(/[\u{1F300}-\u{1FAFF}\u{2600}-\u{27FF}\u{FE00}-\u{FE0F}\u{200D}\u{20E3}\u{E0020}-\u{E007F}]/gu, "");
  // Remover caracteres no legibles (dejar letras, números, puntuación básica, espacios)
  text = text.replace(/[^\p{L}\p{N}\p{P}\p{Zs}\s\-_.[\](){}]/gu, "");
...
function summarizeCandidate(candidate) {
  return {
    ...
    displayName: cleanDisplayName(candidate.displayName),
    ...
    languageHints: Array.isArray(candidate.languageHints) ? candidate.languageHints : [],
    videoSizeBytes: Number(candidate.videoSizeBytes || 0)
  };
}
...
async function probeDirectStream(directUrl, timeoutMs = 6000, options = {}) {
  ...
  try {
    ...
    return {
      ...
      status: response.status,
      contentType,
      contentLength: Number(response.headers.get("content-length") || (response.headers.get("content-range") || "").split("/")[1]) || 0
    };
  } catch (error) {
    return {
      ok: false,
...
function registerPlaybackRoutes(app) {
  ...
  app.post("/api/playback/sessions", async (req, res) => {
    try {
      const session = await createPlaybackSession({
        ...
        forceHls: Boolean(req.body?.forceHls)
      });
  ...
  app.post("/api/playback/auto", async (req, res) => {
    ...
    for (const candidate of orderedWithPreferred) {
      ...
      if (probe.ok) {
        // Estimar resolución por tamaño si M3U no tiene hint
        if (candidate.resolution === 0 && probe.contentLength > 0) {
          const sizeGb = probe.contentLength / (1024 * 1024 * 1024);
          if (sizeGb >= 6) candidate.resolution = 2160;
          else if (sizeGb >= 2) candidate.resolution = 1080;
          else if (sizeGb >= 0.8) candidate.resolution = 720;
          if (probe.contentLength > 0) candidate.videoSizeBytes = probe.contentLength;
        }
        const probedBucket = qualityBucketFromResolution(candidate.resolution);
        upsertValidated({
          bucket: probedBucket,
          mode: "direct",
          streamKind: "direct",
  ...
  app.post("/api/playback/force-hls", async (req, res) => {
    const type = safeText(req.body?.type);
    const itemId = safeText(req.body?.itemId);
    const season = safeText(req.body?.season);
    const episode = safeText(req.body?.episode);
    const requestedSourceKey = safeText(req.body?.sourceKey);

    if (!type || !itemId || !requestedSourceKey) {
      return res.status(400).json({ error: "Debes enviar type, itemId y sourceKey." });
    }

    try {
      const audioSelection = parseAudioSelection(req);
      const aggregated = await fetchAggregatedStreams({
        type, itemId, season, episode, onlyActive: true
      });
      const candidates = buildCandidatesFromResults(aggregated.results, audioSelection);
      const candidate = candidates.find((c) => {
        const key = buildCandidateSourceKey(c);
        return key && key === requestedSourceKey;
      });
      if (!candidate) {
        return res.status(404).json({ error: "No se encontro el candidato solicitado." });
      }

      const waitReadyMs = clamp(Number.parseInt(req.body?.waitReadyMs, 10) || 30000, 8000, 60000);
      let session;
      if (candidate.magnet) {
        session = await createPlaybackSession({
          magnet: candidate.magnet,
          infoHash: candidate.stream?.infoHash,
          displayName: candidate.displayName,
          trackers: getTrackers(candidate.stream),
        ...
        session = await createDirectHlsSession({
        ...
        return res.status(404).json({ error: "El candidato no tiene magnet ni URL directa." });
      }

      const finalSession = await waitForSessionStatusChange(session.id, waitReadyMs);
      if (!finalSession) {
        return res.status(502).json({ error: "Sesion no encontrada tras esperar." });
      }
      if (finalSession.status === "error") {
        return res.status(502).json({ error: finalSession.error || "No se pudo preparar HLS." });
      }
      if (finalSession.status !== "ready") {
        return res.status(504).json({ error: "Timeout esperando sesion HLS." });
      }
      ...
      return res.json({
        ...
        status: "ready",
        streamUrl: sessionPayload.streamUrl,
        streamKind: finalSession.streamKind || "hls",
        sessionId: finalSession.id,
        session: sessionPayload,
        chosen: summarizeCandidate(candidate),
        selectedQuality: qualityBucketFromResolution(candidate.resolution),
        alternatives: [],
        availableQualities: [qualityBucketFromResolution(candidate.resolution)]
      });
    } catch (error) {
      return res.status(500).json({ error: error?.message || "Error forzando HLS." });
    }
  });
