...
const REPORTED_UNAVAILABLE_TTL_MS = 30 * 60 * 1000;
const MAX_CACHE = 500;
...
function markGloballyUnavailable(type, itemId) {
...
async function checkSingleAvailability(type, itemId) {
  ...
  if (cached) {
    if (cached.reportedUnavailable && Date.now() - cached.ts < REPORTED_UNAVAILABLE_TTL_MS) {
      return false;
    }
    if (!cached.reportedUnavailable && Date.now() - cached.ts < CACHE_TTL_MS) {
      return cached.value;
    }
  }
...
function registerAvailabilityRoutes(app) {
  app.post("/api/availability/report-unavailable", (req, res) => {
    try {
      ...
      markGloballyUnavailable(type, itemId);
      return res.json({ ok: true });
    } catch (error) {
      return res.status(500).json({ error: String(error?.message || "Error interno") });
    }
  });
