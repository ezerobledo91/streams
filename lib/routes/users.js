...
function registerUserRoutes(app) {
  ...
  app.delete("/api/users/:username/unavailable", (req, res) => {
    const user = getUser(req.params.username);
    if (!user) {
      return res.status(404).json({ error: "usuario no encontrado" });
    }
    const type = String(req.body?.type || req.query.type || "movie");
    const itemId = String(req.body?.itemId || req.query.itemId || "");
    if (!itemId) {
      return res.status(400).json({ error: "itemId requerido" });
    }
    const updated = clearUnavailable(req.params.username, type, itemId);
    return res.json({ user: updated });
  });
