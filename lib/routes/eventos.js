...
const { eventosState } = require("../state");
...
function registerEventosRoutes(app) {
  app.get("/api/eventos/status", (req, res) => {
    return res.json({
      loadedAt: eventosState.loadedAt,
      categoryCount: eventosState.categories.length,
      channelCount: eventosState.channels.length
    });
  });

  app.get("/api/eventos/categories", (req, res) => {
    return res.json({
      loadedAt: eventosState.loadedAt,
      categories: eventosState.categories
    });
  });

  app.get("/api/eventos/channels", (req, res) => {
    ...
    let pool = eventosState.channels;
    if (category) {
  ...
  app.get("/api/eventos/channels/:id", (req, res) => {
    const id = safeText(req.params.id);
    const item = eventosState.channelById.get(id);
    if (!item) {
  ...
  app.get("/api/eventos/channels/:id/stream", (req, res) => {
    const id = safeText(req.params.id);
    const item = eventosState.channelById.get(id);
    if (!item) {
      return res.status(404).json({ error: "Canal no encontrado." });
...
module.exports = { registerEventosRoutes };
