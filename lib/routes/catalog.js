...
function buildCatalogBrowseCacheKey({ category, query, genre, page, limit, perSourceLimit, loadedAt }) {
  return [
...
function registerCatalogRoutes(app) {
  app.get("/api/catalog/browse", async (req, res) => {
    ...
    const tmdbItems = await withTimeout(
      fetchTmdbCatalogItems(category, query, page, Math.min(90, limit), genre),
