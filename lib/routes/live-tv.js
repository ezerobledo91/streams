...
const { liveTvState, eventosState, marathonState } = require("../state");
...
const remoteSourcesManager = require("../live-tv/remote-sources");

const LIVE_TV_HLS_ROOT_DIR = path.join(PLAYBACK_HLS_DIR, "live-tv");
const LIVE_TV_HLS_IDLE_TTL_MS = 4 * 60 * 1000;
...
function isLiveTvWebPlayableForUi(item) {
  return Boolean(item?.webPlayable) || isLiveTvTranscodeCandidate(item);
}

function buildNameCounter(channels) {
  const counts = new Map();
  for (const item of channels) {
    const key = normalizeLooseText(item?.name);
    if (!key) continue;
    if (!counts.has(key)) {
      counts.set(key, {
        key,
...
function buildSourceCounter(channels) {
  const counts = new Map();
  for (const item of channels) {
    const sourceFile = safeText(item?.sourceFile) || "desconocido";
...
function buildSectionAudit(channels, hiddenIds, normalizedQuery) {
  const all = Array.isArray(channels) ? channels : [];
  const hiddenSet = hiddenIds instanceof Set ? hiddenIds : new Set();
  const queryMatches = normalizedQuery
    ? all.filter((item) => safeText(item?.searchText).includes(normalizedQuery))
    : all;
  const webPlayableMatches = queryMatches.filter((item) => isLiveTvWebPlayableForUi(item));
  const hiddenMatches = queryMatches.filter((item) => hiddenSet.has(item.id));
  const visibleByDefault = webPlayableMatches.filter((item) => !hiddenSet.has(item.id));
  const topNames = buildNameCounter(queryMatches).slice(0, 12);
  const topSourceFiles = buildSourceCounter(queryMatches).slice(0, 12);

  return {
    totalChannels: all.length,
    totalWebPlayableOrProxy: all.filter((item) => isLiveTvWebPlayableForUi(item)).length,
    totalHiddenByUser: all.filter((item) => hiddenSet.has(item.id)).length,
    query: {
      matchCount: queryMatches.length,
      webPlayableOrProxyCount: webPlayableMatches.length,
      hiddenByUserCount: hiddenMatches.length,
      visibleDefaultCount: visibleByDefault.length
    },
    topNames,
    topSourceFiles
  };
...
const LIVE_TV_DEFAULT_QUALITY = "720p";
...
function createLiveTvTranscode(item, quality) {
  ...
  const codecArgs = [
    ...
    "-map", "0:a:0?",
    "-c:v", "libx264",
    "-preset", preset.videoPreset || "veryfast",
    "-tune", "zerolatency",
    "-crf", preset.crf,
    "-b:v", preset.videoBitrate,
    "-maxrate", preset.videoMaxRate,
    "-bufsize", preset.videoBufferSize,
...
function registerLiveTvRoutes(app) {
  ...
  app.get("/api/live-tv/audit", (req, res) => {
    cleanupIdleLiveTvTranscodes(false);
    const rawQuery = safeText(req.query.query);
    const normalizedQuery = normalizeLooseText(rawQuery);
    const prefs = liveTvPreferences.getPreferences();
    const hiddenIds = new Set(Array.isArray(prefs?.hidden) ? prefs.hidden : []);
    ...
    return res.json({
      ...
      sections: {
        tv: buildSectionAudit(liveTvState.channels, hiddenIds, normalizedQuery),
        eventos: buildSectionAudit(eventosState.channels, hiddenIds, normalizedQuery),
        marathon247: buildSectionAudit(marathonState.channels, hiddenIds, normalizedQuery)
      },
      notes: {
        defaultFrontendFilters: ["webOnly=true", "hidden preferences applied"],
        tip: "Si matchCount > visibleDefaultCount, revisa webPlayable/proxy o canales ocultos."
      }
    });
  });
  ...
  app.get("/api/live-tv/active-source", (req, res) => {
    return res.json({
      activeSource: liveTvState.activeSource || "local",
      remoteSources: remoteSourcesManager.getRemoteSourcesStatus()
    });
  });

  app.post("/api/live-tv/active-source", async (req, res) => {
    const source = safeText(req.body?.source);
    if (!["local", "remote", "all"].includes(source)) {
      return res.status(400).json({ error: "source debe ser 'local', 'remote' o 'all'." });
    }

    // Sin cambios si ya es la fuente activa
    if (source === (liveTvState.activeSource || "local")) {
      return res.json({ ok: true, activeSource: source, ...getLiveTvSummary() });
    }

    try {
      // Si cambia a remote/all y no hay caché, descargar primero
      if (source === "remote" || source === "all") {
        const statuses = remoteSourcesManager.getRemoteSourcesStatus();
        const needsDownload = statuses.some((s) => s.enabled && !s.cache.exists);
        if (needsDownload) {
          await remoteSourcesManager.ensureRemoteSourcesCached();
        }
      }

      liveTvPreferences.setActiveSource(source);
      cleanupIdleLiveTvTranscodes(true);
      const summary = loadLiveTvFromDisk();
      return res.json({ ok: true, activeSource: source, ...summary });
    } catch (error) {
      return res.status(500).json({ error: error?.message || "No se pudo cambiar la fuente." });
    }
  });

  app.get("/api/live-tv/remote-sources", (req, res) => {
    return res.json({
      sources: remoteSourcesManager.getRemoteSourcesStatus()
    });
  });

  app.post("/api/live-tv/remote-sources/refresh", async (req, res) => {
    try {
      const results = await remoteSourcesManager.refreshRemoteSources();
      cleanupIdleLiveTvTranscodes(true);
      const summary = loadLiveTvFromDisk();
      return res.json({
      ...
      return res.status(500).json({ error: error?.message || "No se pudo actualizar fuentes remotas." });
    }
  });
