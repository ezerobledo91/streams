...
function buildLiveTvUpstreamHeaders(item, req, targetUrl) {
  ...
  const referer = safeText(item?.httpReferrer);
  if (referer) {
...
function createLiveTvTranscode(item) {
  ...
  const stillRunning = sameSource && existing.ffmpegProcess && existing.ffmpegProcess.exitCode === null;
  if (sameSource && stillRunning && existing.status !== "error") {
...
function registerLiveTvRoutes(app) {
  ...
  app.get("/api/live-tv/channels/:id/hls/:asset", async (req, res) => {
    ...
    // Arrancar ffmpeg si no está corriendo (non-blocking, retorna inmediato)
    try {
      createLiveTvTranscode(item);
    } catch (error) {
    ...
    if (!transcode) {
      return res.status(502).json({ error: "No se pudo iniciar transcode." });
    }
    ...
    const requestedAsset = safeText(req.params.asset) || "index.m3u8";
