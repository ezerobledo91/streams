...
function buildProwlarrQueries({ title, imdbId, resolvedType, seasonNum, episodeNum }) {
  ...
  return [...new Set(queries.map((query) => normalizeReleaseQuery(query)).filter(Boolean))].slice(0, 6);
}
    ...
    queries.map(async (query) => {
      try {
        ...
        const payload = await fetchJson(url.toString(), {
          timeoutMs: PROWLARR_TIMEOUT_MS,
          headers
        });
        const entries = parseProwlarrList(payload).slice(0, PROWLARR_MAX_PER_QUERY);
