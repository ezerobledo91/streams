...
const DEFAULT_LIVE_TV_LISTS_DIR = path.resolve(APP_DIR, "lists");
...
const TMDB_CACHE_TTL_MS = clamp(
  Number.parseInt(safeText(process.env.TMDB_CACHE_TTL_MS), 10) || 60 * 60 * 1000,
  60 * 1000,
  24 * 60 * 60 * 1000
);
const TMDB_CACHE_MAX_ITEMS = clamp(
  Number.parseInt(safeText(process.env.TMDB_CACHE_MAX_ITEMS), 10) || 1500,
  100,
  20000
);
const TMDB_CACHE_PERSIST_ENABLED =
  safeText(process.env.TMDB_CACHE_PERSIST_ENABLED || "true").toLowerCase() !== "false";
const TMDB_CACHE_DIR = path.resolve(
  safeText(process.env.TMDB_CACHE_DIR || path.join(CONFIG_DIR, "tmdb-cache"))
);
const TMDB_CACHE_FILE = path.join(TMDB_CACHE_DIR, "tmdb-cache.json");
...
const PROWLARR_TIMEOUT_MS = clamp(
  Number.parseInt(safeText(process.env.PROWLARR_TIMEOUT_MS), 10) || 8000,
  4000,
  30000
);
...
const PLAYBACK_SESSION_TTL_MS = clamp(
  Number.parseInt(safeText(process.env.PLAYBACK_SESSION_TTL_MS), 10) || 10 * 60 * 1000,
  2 * 60 * 1000,
  60 * 60 * 1000
);
const HOST = safeText(process.env.HOST || "0.0.0.0") || "0.0.0.0";
